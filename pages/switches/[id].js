    ...
    return (
        <Layout>
            <SEO
                ...
                image={{
                    src: s.thumb.formats.medium.url,
                    alt: `${s.name} switch image`,
                    mine: s.thumb.formats.medium.mime,
                    width: s.thumb.formats.medium.width,
                    height: s.thumb.formats.medium.height
                }}
                article_data={{type: 'switches', dateModified: s.updatedAt, datePublished: s.published_at}}
            
            />
            <>
                <div className="flex flex-col lg:py-16 py-0" key={s.id}>
                    <div className="flex md:flex-row flex-col-reverse justify-between md:items-end">
                        <h1>{s.name}</h1>
                        <small>Last edited: {s.updatedAt.slice(0,10).replace(/[-]/g, '/')}</small>
                    </div>
                    <hr></hr>
                    <div className="flex justify-center md:flex-row flex-col mt-8">
                        <div>
                            <Image className="rounded-lg w-1/2 bg-white object-cover" width={500} height={500} src={`${s.thumb.formats.medium.url}`} />
                        </div>
                            ...
                            <div className="rounded-lg p-4 mb-4 shadow-lg" style={{background: "var(--bg-accent)"}}>
                                <section className="mb-2">
                                    ...
                                    <div className="flex flex-row justify-between">
                                        <p>Type:</p>
                                        <p className="font-inter-light capitalize">{s.type.name}</p>
                                    </div>
                                    <div className="flex flex-row justify-between">
                                        <p>Manufacturer:</p>
                                        <p className="font-inter-light">{s.manufacturer.name}</p>
                                    </div>
                                    <div className="flex flex-row justify-between">
                                        <p>Designer:</p>
                                        <p className="font-inter-light">{s.designer}</p>
                                    </div>
                                </section>
                                <section className="mb-2">
                                    ...
                                    <div className="flex flex-row justify-between">
                                        <p>Actuation:</p>
                                        <p className="font-inter-light">{s.actuation}g</p>
                                    </div>
                                    <div className="flex flex-row justify-between">
                                        <p>Bottom Out:</p>
                                        <p className="font-inter-light">{s.bottom_out}g</p>
                                    </div>
                                    <div className="flex flex-row justify-between">
                                        <p>Pre-Travel:</p>
                                        <p className="font-inter-light">{s.pre_travel_tolerance == null ? `${s.pre_travel}mm` : `${s.pre_travel}±${s.pre_travel_tolerance}mm`}</p>
                                    </div>
                                    <div className="flex flex-row justify-between">
                                        <p>Total Travel:</p>
                                        <p className="font-inter-light">{s.total_travel_tolerance == null ? `${s.total_travel}mm` : `${s.total_travel}±${s.total_travel_tolerance}mm`}</p>
                                    </div>
                                </section>
                                <section>
                                    ...
                                    <div className="flex flex-row justify-between">
                                        <p>Stem:</p>
                                        <p className="font-inter-light">{s.material_stem}</p>
                                    </div>
                                    <div className="flex flex-row justify-between">
                                        <p>Top Housing:</p>
                                        <p className="font-inter-light">{s.material_top}</p>
                                    </div>
                                    <div className="flex flex-row justify-between">
                                        <p>Bottom Housing:</p>
                                        <p className="font-inter-light">{s.material_bottom}</p>
                                    </div>
                                    <div className="flex flex-row justify-between">
                                        <p>Spring:</p>
                                        <p className="font-inter-light">{s.spring}</p>
                                    </div>
                                </section>
                            </div>
                            <section className="flex justify-between">
                                <button onClick={copyURL} className={`${copied ? "opacity-40" : ""} transition duration-300 w-1/2 mr-2 h-8 uppercase rounded-xl font-nunito-black text-white`} style={{background: "var(--primary-color)", color: "var(--bg-accent)"}}>{!copied ? "Share" : "Copied"}</button>
